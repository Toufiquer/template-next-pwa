...
const ViewUser: React.FC = () => {
  const { isViewModalOpen, selectedUser, toggleViewModal } = useUserStore();
  ...
  return (
    <Dialog open={isViewModalOpen} onOpenChange={toggleViewModal}>
      <DialogContent>
              ...
              <div className="col-span-2">
                <span
                  className={`px-2 py-1 rounded-full text-xs font-medium ${
                    selectedUser.role === 'admin' ? 'bg-amber-100 text-amber-700' : selectedUser.role === 'moderator' ? 'bg-blue-100 text-blue-700' : 'bg-green-100 text-green-700'
                  }`}
                >
                  {selectedUser.role}
                </span>
        ...
        <DialogFooter>
          <Button className="cursor-pointer border-1 border-slate-400 hover:border-slate-500" onClick={() => toggleViewModal(false)}>
            Close
          </Button>
        </DialogFooter>
      </DialogContent>
